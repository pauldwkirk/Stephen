...
S_n <- function(data, sample_mean, sample_size, num_cols) {
  ...
  if (sample_size > 0) {
    for (index in 1:sample_size) {
      ...
      + (as.matrix(data[index, ] - sample_mean)
...
scale_n <- function(scale_0,
  ...
  + sample_covariance
    ...
    * as.matrix(sample_mean - mu_0) %*% (sample_mean - mu_0)
  )
}
...
variance_posterior <- function(df_0, scale_0, lambda_0, mu_0, data) {
  ...
  if (inherits(data, "data.frame") | ! is.null(dim(data)) ) {
    sample_size <- nrow(data)
...
# data <- as.matrix(c(rnorm(N / 2, -2, 1), rnorm(N / 2, 2, 1))) # hist(data)

data <- matrix(rep(c(rnorm(N / 2, -2, 1), rnorm(N / 2, 2, 1)), d),
               nrow = N,
               ncol = d
               )
...
for (qwe in 1:num_iter) {
  ...
  for (j in 1:k) {
    cluster_data <- data[class_labels == j,] # change for matrix
...
entropy_plot <- ggplot(data = entropy_data, 
                       mapping = aes(x = Index, y = Entropy)
                       ) +
  geom_point() +
...
# --- Check posterior actually used vs expected --------------------------------

# Several inspections are restricted to 1D
if(d == 1){
  p <- postior_sense_check(data[, 1], k, scale_0, mu_0, lambda_0, df_0, 
                         num_points = 1000
                         )
  p
...
annotation_row <- data.frame(Class_label = factor(rep(c(1, 2), N / 2), 
                                                  labels = c("Exp1", "Exp2")
                                                  )
                             )
...
annotation_row[sample(1 : N, N / 5, replace = F), 1] <- NA
