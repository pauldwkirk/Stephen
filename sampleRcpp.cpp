  ...
  for(int i = 1; i < k + 1; i++){
    ...
    exponent = arma::as_scalar(arma::trans(point - mu.slice(i - 1)) 
                                 ...
                                 * (point - mu.slice(i - 1)));
                                 ...
                                 if(outlier && i == k && df > 0.0){
                                   // double comp1 = lgamma((df + d)/2.0);
                                   // double comp2 = - lgamma(df/2.0) - (d/2.0) * log(df);
                                   // double comp3 = - d/2.0 * log(M_PI);
                                   // double comp4 = - 0.5 * log_det;
                                   // double comp5 = - (df + d/2.0) * log(1 + (1/df) * exponent);
                                   // std::cout << comp1 << "\n" << comp2 << "\n" << comp3 << "\n" << comp4 << "\n" << comp5 << "\n\n";
                                   // 
                                   log_likelihood = (lgamma((df + d)/2.0) - lgamma(df/2.0) - d/2.0 * log(df)
                                                       - d/2.0 * log(M_PI) - 0.5 * log_det - (df + d/2.0) * 
                                                         log(1 + (1/df) * exponent));
